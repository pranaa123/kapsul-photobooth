...
export default async function FullGalleryPage({searchParams}:{searchParams:Promise<{event?:string}>}){
  ...
  if(!event)notFound();
  const{data:photos}=await supabase.from("photos").select("id,storage_path,thumbnail_path,created_at").eq("event_id",event.id).neq("status","deleted").order("created_at",{ascending:false}).limit(200);
  ...
  const signed=await Promise.all((photos??[]).map(async photo=>{const[preview,master]=await Promise.all([bucket.createSignedUrl(photo.thumbnail_path||photo.storage_path,3600),bucket.createSignedUrl(photo.storage_path,3600)]);return{preview:preview.data?.signedUrl??"",master:master.data?.signedUrl??""}}));
