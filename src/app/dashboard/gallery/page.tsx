...
export default async function FullGalleryPage({searchParams}:{searchParams:Promise<{event?:string}>}){
  ...
  if(!event)notFound();
  const{data:photos}=await supabase.from("photos").select("id,storage_path,created_at").eq("event_id",event.id).neq("status","deleted").order("created_at",{ascending:false}).limit(200);
  ...
  const gallery=(photos??[]).map((photo,index)=>({...photo,url:signed[index]?.data?.signedUrl??""})).filter(photo=>photo.url);
  return <main className="full-gallery-page">
    ...
    {gallery.length?<section className="full-gallery-grid">{gallery.map((photo,index)=><figure key={photo.id}><img src={photo.url} alt={`Foto tamu ${index+1}`} loading="lazy"/><figcaption>#{String(gallery.length-index).padStart(3,"0")} · {new Date(photo.created_at).toLocaleString("id-ID",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"})}</figcaption></figure>)}</section>:<p className="gallery-empty">Belum ada file foto yang dapat ditampilkan.</p>}
  </main>;
}
