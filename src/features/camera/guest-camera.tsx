...
type GuestEvent={name:string;startsAt?:string|null;maxPhotosPerDevice?:number;eventType?:string;welcomeMessage?:string;completionMessage?:string;allowMessages?:boolean;slug?:string;publicToken?:string};
...
export function GuestCamera({event={name:"Rania & Dava",startsAt:"2026-08-12T16:00:00+08:00",maxPhotosPerDevice:10,eventType:"Pernikahan"}}:{event?:GuestEvent}){
  ...
  if(step==="done")return <div className="guest done-screen"><Brand light/><div className="done-mark"><Check/></div><span className="guest-eyebrow">BERHASIL DIKIRIM</span><h1>TERIMA KASIH<br/>SUDAH <em>HADIR.</em></h1><p>{event.completionMessage||`${submittedTotal} dari ${limit} foto dari perangkat ini sudah diterima oleh ${event.name}.`}</p>{submittedTotal<limit?<button onClick={()=>{setPhotos([]);setGuestName("");setMessage("");reserved.current=null;setStep("welcome")}}>Ambil foto lagi · {limit-submittedTotal} tersisa</button>:<small>Batas {limit} foto untuk acara ini sudah tercapai.</small>}{toast}</div>;
